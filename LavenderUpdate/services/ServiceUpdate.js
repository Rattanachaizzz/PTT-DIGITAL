...
module.exports.DownloadFile = async (downloadTime, messageID, siteCode, socket) => {
  try {
    ...
    if (dataSelectActions.length !== 0) {
      ...
      //await exec('sudo apt update --allow-insecure-repositories', async (error, stdout, stderr) => {
      //if (error)
      //console.log(error)
      //});
      let dataDownload = []
      let responeDownloadAll = await Promise.all(dataSelectActions.map(async (data, i) => {
        let responeDownloadMain = await new Promise(async (resolve, reject) => {
          exec(`cd /var/cache/apt/archives && ${data.command}`, async (error, stdout, stderr) => {
            // console.log(data.command)
            // console.log("error" + error)
            // console.log("stdout" + stdout)
            // console.log("stderr" + stderr)
            if (fs.existsSync(`/var/cache/apt/archives/${data.service_name}_${data.version}_amd64.deb`)) {
              resolve({
                status_code: 2,
              ...
              resolve({
                ...
                error: stdout //"Download failed."
              })
            }
          })
        })
        await baseControl.serviceBase.UpdateStatusCode(responeDownloadMain, data) // update status db.sqlite
        if (responeDownloadMain.status_code === -1) {
          console.log("responeDownloadMain.error",responeDownloadMain.error)
          dataDownload.push({
            version: data.version,
            downloadDate: moment().format('YYYY-MM-DD HH:mm:ss'),
            downloadStatus: "failed",
            error: responeDownloadMain.error
          })
        ...
        if (Number(data.rollback_enable) === 1) {
          ...
          if (checkFileRollback === true) {
            ...
            let responeDownloadRollback = await new Promise(async (resolve, reject) => {
              exec(`cd /var/cache/apt/archives &&  ${data.command.split('=')[0]}=${data.rollback_version}`, async (error, stdout, stderr) => {
                if (fs.existsSync(`/var/cache/apt/archives/${data.service_name}_${data.rollback_version}_amd64.deb`)) {
                  resolve({
                    status_code: 2,
                    Description: "Waiting for trigger/execute",
                    error: ""
            ...
            if (responeDownloadRollback.status_code === -1) {
              console.log("responeDownloadRollback.error",responeDownloadRollback.error)
              dataDownload.push({
                version: data.rollback_version,
                downloadDate: moment().format('YYYY-MM-DD HH:mm:ss'),
                downloadStatus: "failed",
                error: responeDownloadRollback.error
              })
...
module.exports.checkPurgeService = async (nameService) => {
  return await new Promise((resolve, reject) => {
    exec(`sudo apt list --installed ${nameService}`, (error, stdout, stderr) => {
      if (!stdout.includes(nameService)) {
        resolve({
          status_code: 0,
...
async function RollbackInstall(dataSelectActions, messageID, socket) {
  if (dataSelectActions.length !== 0) {
    let rollbackStatus = await new Promise((resolve, reject) => {
      exec(`apt -y  install /var/cache/apt/archives/${dataSelectActions[0].service_name}_${dataSelectActions[0].rollback_version}_x64.deb`, (error, stdout, stderr) => {
        if (error) {
          resolve({
            status_code: -2,
