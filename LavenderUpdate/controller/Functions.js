...
module.exports.ScheduleJob = async (socket, req) => {
  ...
  try {
    ...
    if (req.messageType === `request`) {
      ...
      const downloadDate = moment(req.action[0].downloadDate)
      let dateNow = moment(`${moment().format('YYYY-MM-DD HH:mm:ss')}`)
      if (downloadDate.isBefore(dateNow)) {
        socket.emit('response', {
          clientID: siteCode,
          messageID: messageID,
          messageType: "Response",
          actionID: req.action[0].actionID,
          status: `denied`,
          timeStamp: moment().format('YYYY-MM-DD HH:mm:ss'),
          error: 'The server time is less than the time at client.'
        })
      } else {
        ...
        if (dataSelectActions[0].operation === "apt-install") {
          exec(command, (error, stdout, stderr) => {
           if (stdout.includes("Setting up")) {
              ...
              exec(`rm /var/cache/apt/archives/${dataSelectActions[0].service_name}*`)
              return
            } else {
              if (error) {
                socket.emit('response', {
    ...
    await baseControl.serviceBase.getAction(socket);
  } else if (req.messageType === `deleteAction`) {
    await baseControl.serviceBase.deleteAction(socket);
  } else if (req.messageType === `updateAction`) {
    await baseControl.serviceBase.updateAction(socket, 2, req.action[0].actionID);
  } else {
    socket.emit('response', {
      messageID: messageID,
      messageType: messageType,
